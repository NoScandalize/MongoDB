...
router.post('/', verifyToken, upload.fields([{ name: "photos" }]), async (req, res) => {
    ...
    // validations
    if(!title) {
        return res.status(400).json({ error: "É necessário preencher o título da festa!" })
    }

    if(!description) {
        return res.status(400).json({ error: "É necessário preencher a descrição da festa!" })
    }

    if(!partyDate) {
        return res.status(400).json({ error: "É necessário preencher a data da festa!" })
    }
    ...
    try {
        ...
        try {
            ...
            return res.status(201).json({ error: null, message: "Evento criado com sucesso!", data: newParty })
...
router.get("/userparties", verifyToken, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            return res.status(200).json({ error: null, userParties: userParties })
...
router.delete('/', verifyToken, async (req, res) => {
    ...
    if(!partyId) {
        return res.status(400).json({ error: "Ocorreu algum erro, tente novamente mais tarde!" })
    }

    const party = await Party.findOne({ _id: partyId })

    if(!party) {
        return res.status(400).json({ error: "Evento não encontrado ou inexistente!" })
    }
...
router.put('/', verifyToken, upload.fields([{ name: "photos" }]), async (req, res) => {
    ...
    try {
        ...
        return res.json({ error: null, message: "Evento atualizado com sucesso!", data: updatedParty })
